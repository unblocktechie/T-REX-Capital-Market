...
2. The frontend validates the connected wallet, approved issuer wallet, and Arc Testnet chain.
3. The issuer signs `deployTREXSuite` through the connected wallet.
4. The frontend waits for a successful Arc Testnet receipt and validates the returned transaction hash.
