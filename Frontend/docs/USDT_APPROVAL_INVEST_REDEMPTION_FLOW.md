# USDC Approval, Invest, and Redemption Flow

## Final ownership/signing rule

| Action | USDC direction | USDC owner that approves Platform Controller | Action transaction signer |
| --- | --- | --- | --- |
...
1. Read the investor's live USDC allowance for the Platform Controller.
...
3. After issuer approval, read the issuer organization's live USDC allowance and balance.
...
9. The contract burns the investor's tokens and transfers USDC directly from issuer to investor atomically.
10. The frontend does not submit a separate USDC transfer and does not custody redemption funds.
...
- Allowances are read from the USDC contract; page refresh does not rely on a local "approved" flag.
...
- Invest re-checks investor USDC balance and allowance immediately before submission.
- Redeem re-checks issuer USDC balance and allowance immediately before submission.
