# Purchase USDC Approval Flow

The investor purchase flow now separates USDC spending approval from the token purchase transaction.

## Step 1 — Allow USDC Spending

- Reads the current USDC allowance directly from the payment-token contract.
...
- If approval is required, **Allow USDC Spending** submits only `USDC.approve(PlatformController, MAX_UINT256)` and waits for a successful receipt.
...
- It checks that the existing allowance covers the quoted USDC amount, then submits only `PlatformController.buy(...)`.
...
The page re-reads allowance from the USDC contract. The app grants `MAX_UINT256`; a persistent approval remains recognized across refreshes and future purchases. A new approval is only requested if the permission is absent/revoked/no longer sufficient.
