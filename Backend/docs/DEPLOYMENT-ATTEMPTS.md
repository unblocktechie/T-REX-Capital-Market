...
```json
{ "chainId": 5042, "walletAddress": "0x...", "idempotencyKey": "unique-client-value" }
```
...
Request:
```json
{ "transactionHash": "0x...", "chainId": 5042, "walletAddress": "0x..." }
```
`200`:
```json
...
deploy block) to avoid scanning Arc Network from genesis.
...
```json
{
  ...
  "data": {
    ...
    "attempt": {
      ...
      "chainId": 5042, "walletAddress": "0x...", "expiresAt": null,
...
Verification uses the configured Arc Network RPC (`BLOCKCHAIN_RPC_URL`, chain `5042`) and the
...
BLOCKCHAIN_NETWORK_NAME=arc-mainnet
