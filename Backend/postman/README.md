...
7. Use **Admin Organization Review** with `adminToken` to list pending submissions, load the complete application, preview/download its documents, and approve or reject it. The rejection example sets `rejectionReason`; after one issuer revision and resubmission, a second rejection returns `canResubmit: false`.
