...
2. The frontend validates the connected wallet, approved issuer wallet, and Arc chain.
3. The issuer signs `deployTREXSuite` through the connected wallet.
4. The frontend waits for a successful Arc receipt and validates the returned transaction hash.
