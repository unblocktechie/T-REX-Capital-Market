...
const makeService = ({ verifyError = null, interestStatus = 'claimSubmitted' } = {}) => {
  const state = { rows: [], verificationCalls: 0, interestStatus, history: [] };
  const context = {
    ...
    countryNumericCode: '356', countryActive: true, countryDeleted: false,
  };
  const repository = {
  ...
  const verifier = {
    inspectRegistryState: async () => ({ contains: false, matches: false, issuerIsAgent: true }),
